...
Still built on **ECM-Calibration v8.0.0**. No changes to the algorithms or to
any numerical result — verified against the bundled tutorial fixture, whose
median eigenvalue ratio is unmoved. This release adds a dark theme.

### Added
- **Dark theme**, covering the application chrome *and* the Plotly figures.
  Switch it in the toolbar menu (⋮ → Settings → Appearance), which also
  offers "System" to follow the operating system. The toolbar menu is
  visible again for this: it was set to `minimal`, which hid the theme
  switch along with the developer options. It now runs in `viewer` mode, so
  Rerun, Clear cache and Deploy stay hidden.
- **`streamlit_app/utils/theme.py`**, one place holding every colour the GUI
  uses, in two palettes. The Plotly helpers, the injected CSS and the inline
  HTML blocks all read from it instead of carrying their own hex values.
- **A dark variant of the logo**, generated by `tools/build_icons.py`.
  Removing the white background is not enough on its own: the anvil and the
  MM·FORGE wordmark are drawn in the MMForge slate, which is effectively
...
- **Two trace colours were unusable on any dark background.** The 16-colour
  ...
  (2.28, barely better). The dark sequence replaces both, and every one of
  its 16 entries clears contrast 4.5. Blue gets a higher lightness floor
  than the other hues, blue-on-black being the hardest combination the eye
  has to resolve.
- **Table rows now visibly highlight under the pointer in dark mode.** The
  highlight was a 5% wash tuned for a dark slate over white; the same 5% of
  a light slate over a near-black row was nothing.

### Notes
- **The light theme is unchanged.** Not "essentially unchanged": the
  generated stylesheet was diffed line by line against the one that shipped
  in v2.1.0, and 20 figure types were rebuilt and compared as Plotly JSON
  against their v2.1.0 output. Both are identical.
- **Figures export in whichever theme is on screen.** The camera button in a
  chart's toolbar runs in the browser and saves what it sees; Plotly offers
  no way to recolour that from the application. For a figure going into a
  paper, switch to the light theme first — the charts repaint on the next
  interaction.
- **Charts follow a theme switch on the next rerun, not instantly.**
  Changing the theme restyles the page in the browser without re-running the
  application, so figures already on screen keep their palette until
  something else causes a rerun — changing page, or using any control.
